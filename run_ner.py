...
class ModelArguments:
    ...
    use_crf_head: bool = field(
        default=False,
        metadata={
            "help": "If set to `True`, a CRF (Conditional Random Field) layer will be added on top of the model."
        },
    )
...
class DataTrainingArguments:
    ...
    validation_split_ratio: Optional[float] = field(
        ...
        metadata={
            "help": (
                "The proportion of the training set to use as a validation set "
                "if a separate validation set is not provided. If set to `None`, it is not allowed to split "
                "the training set and a ValueError will be thrown."
            )
        },
    )
...
def main():
    ...
    if training_args.do_eval:
        if "validation" not in raw_datasets:
            if data_args.validation_split_ratio is not None:
                if 0 < data_args.validation_split_ratio < 1:
                    splited_train_dataset = raw_datasets["train"].train_test_split(
                        test_size=data_args.validation_split_ratio,
                        seed=training_args.seed,
                    )
                    raw_datasets["train"] = splited_train_dataset["train"]
                    raw_datasets["validation"] = splited_train_dataset["test"]
                else:
                    raise ValueError("validation_split_ratio must be between 0 and 1")
            else:
                raise ValueError(
                    "--do_eval requires a validation dataset, either provide a validation set or specify a validation_split_ratio between 0 and 1"
                )
